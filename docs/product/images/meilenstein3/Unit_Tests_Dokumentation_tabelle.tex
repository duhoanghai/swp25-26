...
\hline
\textbf{Testname} & \textbf{Was wurde getestet?} & \textbf{Status} \\ \hline

\textbf{AnswerTest} & 
Prüft, ob die \texttt{Answer}-Klasse Werte wie \texttt{id} und \texttt{result} korrekt speichert. &
Bestanden\\ \hline

\textbf{QuestionTest} & 
Testet, ob die verschiedenen Frageklassen ihre Felder korrekt speichern.
 & 
Bestanden \\ \hline


\textbf{QuestionTypeAdapterTest} &
Prüft, ob der Adapter den richtigen Fragetyp aus der JSON-Datei erkennt und
Fehler bei fehlendem oder falschem \texttt{type} auslöst. &
Bestanden \\ \hline



\end{tabular}
\caption{Tabelle der durchgeführten Unit-Tests}
\end{table}
